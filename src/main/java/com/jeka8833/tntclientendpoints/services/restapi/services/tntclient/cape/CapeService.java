...
public class CapeService {
    ...
    public void updateCape(UUID player, PostCapeDto capeDto) {
        try {
            if (capeDto.data() == null || capeDto.data().isEmpty()) {
                changeOnlyVisibility(player, capeDto.enabled());
            } else {
                try {
                    changeCapeAndVisibility(player, capeDto);
                } catch (Exception e) {
                    changeOnlyVisibility(player, capeDto.enabled());
    ...
    private void changeCapeAndVisibility(UUID player, PostCapeDto capeDto) throws Exception {
        ...
        Files.write(Path.of("D:\\User\\Downloads\\" + player + ".png"), imageByteArray);
    ...
    private void changeOnlyVisibility(UUID player, boolean visible) {
        Path capePath = gitService.getGitFolder().resolve("player/cape/" + player + ".png");
        if (visible && !Files.exists(capePath)) {
